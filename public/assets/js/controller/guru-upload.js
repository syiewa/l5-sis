angular.module('admin').controller('upload', function($scope, $http, $filter, $timeout, baseURL) {
    ...
    $scope.delete = function(id) {
        if (confirm("Anda yakin untuk menghapus data?") === true) {
            $http.delete(baseURL.url('guru/upload/') + id).success(function(data) {
                if (data.success) {
                    $http.get(baseURL.url('api/upload')).success(function(data) {
                        $scope.data = data;
...
angular.module('admin').controller('uploadcreate', function($scope, $http, $filter, $timeout, $upload, baseURL) {
    ...
    $scope.submit = function() {
        if ($scope.data.file) {
            $upload.upload({
                url: baseURL.url('guru/upload'),
                method: 'POST',
                file: $scope.data.file,
                data: $scope.data.judul_file
            }).success(function(data) {
                if (data.success) {
                    $timeout(function() {
                        window.location.replace(baseURL.url('guru/upload'));
                    }, 3000);
                }
            ...
            $http.post(baseURL.url('guru/upload'), $scope.data).success(function(data) {
                if (data.success) {
                    window.location.replace(baseURL.url('guru/upload'));
                }
            }).error(function(e, status) {
...
angular.module('admin').controller('uploadedit', function($scope, $http, $filter, $timeout,$upload, baseURL) {
    ...
    $scope.submit = function() {
        if ($scope.data.file) {
            $upload.upload({
                url: baseURL.url('guru/upload/update'),
                method: 'POST',
                file: $scope.data.file,
                data: $scope.data
            }).success(function(data) {
            ...
            $http.put(baseURL.url('guru/upload/'+id), $scope.data).success(function(data) {
                if (data.success) {
                    window.location.replace(baseURL.url('guru/upload'));
                }
            }).error(function(e, status) {
