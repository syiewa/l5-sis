...
app.controller('beritacreate', function($scope, $http, $filter, $timeout, $upload) {
    $scope.data = {};
    $scope.submit = function() {
        ...
        if ($scope.data.foto) {
            ...
            $http.post('http://laravel.dev/admin/berita/', $scope.data).success(function(data) {
                if (data.success) {
                    $timeout(function() {
                        window.location.replace('http://laravel.dev/admin/berita');
                    }, 3000);
...
app.controller('pengumuman', function($scope, $http, $filter, $timeout) {
    $scope.data = {};
    $scope.alerts = [];
    $scope.closeAlert = function(index) {
        $scope.alerts.splice(index, 1);
    };
    $http.get('http://laravel.dev/api/pengumuman').success(function(data) {
        $scope.data = data;
        $scope.totalItems = $scope.data.length;
        $scope.currentPage = 1;
        $scope.numPerPage = 5;
    ...
    $scope.delete = function(id) {
        if (confirm("Anda yakin untuk menghapus data?") === true) {
            $http.delete('http://laravel.dev/admin/pengumuman/' + id).success(function(data) {
                if (data.success) {
                    $http.get('http://laravel.dev/api/pengumuman').success(function(data) {
                        $scope.data = data;
                        $scope.alerts.push({type: 'success', msg: 'Data Berhasil Dihapus'});
...
app.controller('pengumumancreate', function($scope, $http, $filter) {
    $scope.data = {};
    $scope.submit = function() {
        $scope.data['isi'] = CKEDITOR.instances.editor1.getData();
        $http.post('http://laravel.dev/admin/pengumuman', $scope.data).success(function(data) {
            if (data.success) {
                window.location.replace('http://laravel.dev/admin/pengumuman');
            }
        });
    }
});

app.controller('pengumumanedit', function($scope, $http, $filter, $timeout) {
    $scope.data = {};
    var id = $filter('_uriseg')(4);
    $http.get('http://laravel.dev/api/pengumuman/' + id).success(function(data) {
        $scope.data = data;
    })
    $scope.submit = function(id) {
        $scope.data['isi'] = CKEDITOR.instances.editor1.getData();
        $http.put('http://laravel.dev/admin/pengumuman/' + id, $scope.data).success(function(data) {
            if (data.success) {
                $timeout(function() {
                    window.location.replace('http://laravel.dev/admin/pengumuman');
                }, 3000);
